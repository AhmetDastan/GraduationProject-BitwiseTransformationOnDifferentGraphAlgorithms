...
int Lbound;
int Ubound; 
block_t * Color; // !!! donusturduk // color array for each vertex
int * SizeG;  
block_t * Best_Color ; // !!! donusturduk // color array for each vertex
...
void Initializing()
{
     ...
     while ( ! FIC.eof() )
     {
           ...
           if ( strcmp(StrReading, "p" )==0 )
           {
              ...
              config.init(N); // Initialize the configuration with vertex amount
              Color = new block_t[config.totalBlockAmount];
              Best_Color = new block_t[config.totalBlockAmount];
...
void Build_Delta_Matrix( )
{
  ...
  for( i = 0 ; i < N ; i++ )
    for( j = 0 ; j < i ; j++ )
        if( Edge[ i ][ j ] != 0 )
          { 
            Delta_Matrix[ i ][ GET_VALUE(Color, j ) ] += 1;
            Delta_Matrix[ j ][ GET_VALUE(Color, i ) ] += 1;
            if( GET_VALUE(Color, i ) == GET_VALUE(Color, j ) ) 
                f += 1;
          }  
...
{   
     ...
     //for(i=0;i<N;i++) SET_VALUE(Color, i, Color_into[i]); //Color[i] = Color_into[i]; 
     bulk_compress_from_ints(Color, Color_into);  
     Build_Delta_Matrix( );
    ...
    // cout << "---------------------------------------------------" << endl;
     ...
     while( non_improve < alpha )
       ...
       // a. evaluating the 1-move neighborhood N1
          for( x = 0 ; x < N ; x++ )
            if( Delta_Matrix[ x ][ GET_VALUE(Color, x)] )
              {
                cs++;
                for( v = 0 ; v < K ; v++ )
                  if( (v != GET_VALUE(Color, x)) && (SizeGroup[GET_VALUE(Color, x)] > Lbound) && (SizeGroup[v] < Ubound) )
                    {
                      num ++ ;
                      delt = Delta_Matrix[ x ][ v ] - Delta_Matrix[ x ][ GET_VALUE(Color, x) ];
           ...
           // b. evaluating the 2-move neighborhood N2      
            for( x = 0 ; x < N ; x++ )
             if(Delta_Matrix[ x ][GET_VALUE(Color,x)])
              {
                for( y = 0 ; y < N ; y++ )
                  if((GET_VALUE(Color,x) !=  GET_VALUE(Color, y)) && (x!=y) )
                  {
                    num2 ++ ;
                    delt = (Delta_Matrix[ x ][GET_VALUE(Color, y)] - Delta_Matrix[ x ][ GET_VALUE(Color, x) ]) + (Delta_Matrix[ y ][GET_VALUE(Color, x)] - Delta_Matrix[ y ][ GET_VALUE(Color, y)]) - 2*Edge[x][y] ;
                    if( (TabuTenure[ x ][ GET_VALUE(Color, y) ] <= iter) && (TabuTenure[ y ][ GET_VALUE(Color, x) ] <= iter) ) // if this is not tabued
                    {
                    ...
                    }else if( (TabuTenure[ x ][ GET_VALUE(Color, y) ] > iter) || (TabuTenure[ y ][ GET_VALUE(Color, x) ] > iter)) // if it is tabu
                    {
           ...
           if( (swap_num_tabu_best > 0 && swap_tabu_best_delta < swap_best_delta && ( f + swap_tabu_best_delta < f_best )) || ( num_tabu_best > 0 && tabu_best_delta < best_delta && ( f + tabu_best_delta < f_best ) ) || (num_best + swap_num_best) == 0 )  // aspiration criterion 
                   {
                     ...
                     if( tabu_best_delta <= swap_tabu_best_delta )
                     {
                       ...
                       old_color = GET_VALUE(Color, tabu_best_x[ select ]) ;
                       One_Move_Update_Delta_Matrix( tabu_best_x[ select ], old_color, tabu_best_v[ select ] );
                       SET_VALUE(Color, tabu_best_x[ select ], tabu_best_v[ select ]);
                     ...
                     else
                     {
                       ...
                       old_color1 = GET_VALUE(Color, swap_tabu_best_x[ select ]) ;
                       old_color2 = GET_VALUE(Color, swap_tabu_best_y[ select ]) ;
                       ...
                       SET_VALUE(Color, swap_tabu_best_x[ select ], old_color2);
                       SET_VALUE(Color, swap_tabu_best_y[ select ], old_color1);
              ...
              else // choose the best non tabu move
                   {
                     if(best_delta <= swap_best_delta)
                     {
                       ...
                       old_color = GET_VALUE(Color, best_x[ select ]) ;              
                       One_Move_Update_Delta_Matrix( best_x[ select ], old_color, best_v[ select ] );
                       SET_VALUE(Color, best_x[ select ], best_v[ select ]);
                     ...
                     else 
                     {
                       ...
                       old_color1 = GET_VALUE(Color, swap_best_x[ select ]) ;  
                       old_color2 = GET_VALUE(Color, swap_best_y[ select ]) ; 
                       ...
                       SET_VALUE(Color, swap_best_x[ select ], old_color2);
                       SET_VALUE(Color, swap_best_y[ select ], old_color1);
                 ...
                 if( f <= f_best )
                   {
                     if( f < f_best )
                       {
                         f_best = f;
                         bulk_copy_compressed_values(Best_Color, Color);
   ...
   for(i=0;i<N;i++) Color_into[i] = GET_VALUE(Best_Color, i ); //Color_into[i] = Best_Color[ i ] ;
...
void Outresulting(Statistics DATE[], string filename, string File_Name, int runs)
{
    int i,j;
    FILE *fp; 
   	char buff[80];
    ...
    fp=fopen(buff,"a+");
    fprintf(fp,"Bitwise: %s   %d    %d    %lf    %d    %lf \n", File_Name.c_str(), N, min_k, K_SUM, N_hit, time_avg );  
	fclose(fp);         
}
/*****************************************************************************/
...
void Perturbation(Solution &S, int SizeGroup[], int theta)
{
     ...
     for(i=0;i<N;i++) SET_VALUE(Color, i, S.p[i]);
     Build_Delta_Matrix( );
     //printf(" f = %d  \n",f);
     //printf("\n\n");
     //cout << endl << "One_Move :       iter       f       f_best      time " << endl;
       ...
       // b. searching the 2-move neighborhood N2  
          for( x = 0 ; x < N ; x++ )
           ...
           // printf("\n f= %d  |c(s)| = %d   |N1| = %d   |N2| = %d ",f, cs, num, num2); 
              ...
              delt = (Delta_Matrix[ x ][GET_VALUE(Color, y)] - Delta_Matrix[ x ][ GET_VALUE(Color, x) ]) + (Delta_Matrix[ y ][GET_VALUE(Color, x)] - Delta_Matrix[ y ][ GET_VALUE(Color, y)]) - 2*Edge[x][y] ;
              f += delt ;
              old_color1 = GET_VALUE(Color, x) ;  
              old_color2 = GET_VALUE(Color, y) ; 
                                   
              One_Move_Update_Delta_Matrix( x, old_color1, old_color2 );
              One_Move_Update_Delta_Matrix( y, old_color2, old_color1 );
                       
              SET_VALUE(Color, x, old_color2);
              SET_VALUE(Color, y, old_color1);

              iter ++ ;
             // total_time = (clock() - starting_time )/CLOCKS_PER_SEC ;                                 
     }
   for(i=0;i<N;i++) S.p[i] = GET_VALUE(Color, i);
   for(i=0;i<K;i++) SizeGroup[i]=0;
...
int Perturb_TS(int Color_into[], int SizeGroup[], int *value, int Iter_max, int L, int *f_cb )
{
     ...
     for(i=0;i<N;i++) SET_VALUE(Best_Color, i, Color_into[i]); ////Best_Color[i] = Color_into[i];
     */bulk_compress_from_ints(Color, Color_into);  
     bulk_compress_from_ints(Best_Color, Color_into);  
     Build_Delta_Matrix();
     f_best = f ;
     //printf("\n\n");
     //cout << endl << "One_Move :       iter       f       f_best      time " << endl;
   ...
   (*f_cb) = f_best;  
   for(i=0;i<N;i++) Color_into[i] = GET_VALUE(Color, i); //Color_into[i] = Color[i];
   for(i=0;i<K;i++) SizeGroup[i]=0; 
   for(i=0;i<N;i++) SizeGroup[GET_VALUE(Color, i)] ++; 
   (*value) = f; 
   return f;  
}
...
int proof(Solution &GS)
{
    ...
    //i dont need this part about solution
    if( flag == 0 && f == 0) 
    {   
        printf("\n K = %d  f = %d \n",K1,f);
...
{ 
     ...
     outfilename = argv[2];
     */
      File_Name = "DSJC125.9.col";
      outfilename = "resultFile";
    

     Initializing();
     if(N<=500) time_limit = 3;
